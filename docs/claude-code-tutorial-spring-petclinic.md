...
> **💡 Claude Code will ask for permission when it needs to:**
>
...
## Step 10: Commit Your Changes (if using Git)
...
## Key Differences from the Excalidraw Tutorial

| Aspect | Excalidraw (TypeScript/React) | Spring Petclinic (Java/Spring) |
|--------|-------------------------------|-------------------------------|
| Package manager | Yarn | Maven (./mvnw) |
| Run command | `yarn start` | `./mvnw spring-boot:run` |
| Default port | 3000 | 8080 |
| Test command | `yarn test:update` | `./mvnw test` |
| Lint/format | `yarn fix` | `./mvnw spring-javaformat:apply` |
| CLAUDE.md | Pre-existing | Created with `/init` |
| Architecture diagram | `.excalidraw` file | Mermaid class diagram |
| Feature built | Rainbow toolbar animation | REST API + Statistics dashboard |
| Template engine | React components | Thymeleaf HTML |
| Database | N/A (client-side) | H2 in-memory (auto-populated) |
